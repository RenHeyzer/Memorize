...
import dev.renheyzer.memorize.feature.core.statistics.di.StatisticsDependenciesImpl
...
class AppDependenciesImpl(context: Context) : AppDependencies {
    ...
    override fun numbersDependencies(): NumbersDependencies =
        NumbersDependenciesImpl(dispatchers, firebaseAuth, firebaseFirestore)

    override fun cardsDependencies(): CardsDependencies =
        CardsDependenciesImpl(dispatchers, firebaseAuth, firebaseFirestore)

    override fun statisticsDependencies(): StatisticsDependencies =
        StatisticsDependenciesImpl(
            dispatchers,
            numbersDependencies().numbersRemoteDataSource,
            cardsDependencies().cardsRemoteDataSource
        )
