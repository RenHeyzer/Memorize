...
//        countdownTimerManager.setDuration(params.recallTimeSeconds * 1000L)
    ...
    fun onIntent(intent: RecallIntent) {
        when (intent) {
            ...
            RecallIntent.OnBackClick -> _recallState.update { state ->
                state.copy(isShowExitDialog = true)
            }

            RecallIntent.OnExitConfirmed -> {
                _recallState.update { state -> state.copy(isShowExitDialog = false) }
                scope.launch { _actions.send(RecallAction.NavigateHome) }
            }

            RecallIntent.OnExitDismissed -> _recallState.update { state ->
...
sealed interface RecallIntent {
    ...
    data object OnBackClick : RecallIntent
    data object OnExitConfirmed : RecallIntent
    data object OnExitDismissed : RecallIntent
}

sealed interface RecallAction {
    data object NavigateHome : RecallAction
