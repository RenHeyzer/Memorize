...
import dev.renheyzer.memorize.feature.core.cards.presentation.store.recall.RecallStore
...
class DefaultRecallComponent(
    ...
    private val scope = coroutineScope(context = env.mainContext + SupervisorJob())
    ...
    override val uiState: StateFlow<RecallUiState> = store.uiState
        ...
        scope.launch {
            store.actions.collect { action ->
                when (action) {
                    ...
                    RecallAction.NavigateHome -> navigateHome()
                }
            }
        }
    }
