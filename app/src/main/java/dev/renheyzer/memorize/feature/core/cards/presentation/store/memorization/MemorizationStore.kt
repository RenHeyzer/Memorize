...
class MemorizationStore(
    ...
    fun onIntent(intent: MemorizationIntent) {
        when (intent) {
            ...
            MemorizationIntent.BackClicked -> _uiState.update { state ->
                state.copy(isShowExitDialog = true)
            }

            MemorizationIntent.ExitConfirmed -> {
                _uiState.update { state -> state.copy(isShowExitDialog = false) }
                scope.launch { _actions.send(MemorizationAction.NavigateHome) }
            }

            MemorizationIntent.ExitDismissed -> _uiState.update { state ->
...
sealed interface MemorizationIntent {
    ...
    data object BackClicked : MemorizationIntent
    data object ExitConfirmed : MemorizationIntent
    data object ExitDismissed : MemorizationIntent
}

sealed interface MemorizationAction {
    data object NavigateHome : MemorizationAction
