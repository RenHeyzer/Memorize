...
fun MemorizationScreen(
    ...
    if (state.isShowExitDialog) {
        ConfirmationDialog(
            onDismiss = { component.onIntent(MemorizationIntent.ExitDismissed) },
            onConfirm = { component.onIntent(MemorizationIntent.ExitConfirmed) }
        )
    }
    ...
    Scaffold(
        ...
        topBar = {
            MemorizeTopBar(
                ...
                onBackClick = { component.onIntent(MemorizationIntent.BackClicked) }
            )
        }
    ) { innerPadding ->
        MemorizationContent(
