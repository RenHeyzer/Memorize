...
class FakeNumbersRepository : NumbersRepository {
    ...
    private var cachedResult: NumbersResult? = null

    override suspend fun saveGameResult(result: NumbersResult): Either<AppError, Unit> {
        cachedResult = result
        delay(1000)
        return Either.Right(Unit)
    }

    override suspend fun getGameResult(): Either<AppError, NumbersResult> {
        delay(1000)
